...
HEADERS = {"User-Agent": "ia-decisor-literature-graph/1.1"}
...
def load_sources() -> pd.DataFrame:
    ...
    required = [
        ...
        "publication", "doi_url", "origin", "level",
    ]
...
with tab_graph:
    if selected_articles:
        components.html(make_graph(selected_articles, direct, coupling, version_edges, show_topics, edge_types), height=740, scrolling=False)
        st.caption("Rojo: cita directa. Gris discontinuo: referencias compartidas. Amarillo discontinuo: versión equivalente. Color del nodo: subtema.")
    else:
        st.warning("No hay artículos con los filtros actuales.")

with tab_sources:
    st.subheader("Corpus maestro")
    display_df = SOURCES_DF[["priority", "title", "authors", "year", "subtopic", "publication", "doi_url", "origin", "level"]].copy()
    display_df.columns = ["Prioridad", "Título", "Autores", "Año", "Subtema", "Publicación / medio", "DOI / URL", "Origen documental", "Nivel"]
...
with tab_refs:
    ...
    if options:
        ...
        st.markdown(f"**Publicación:** {chosen['publication']}  \n**Origen documental:** {chosen['origin']}  \n**DOI/URL:** {chosen['doi_url'] or 'No registrado'}")
...
st.divider()
st.caption("Fuente maestra: sources.csv. Relaciones dinámicas: OpenAlex. Fallback: citas cruzadas verificadas manualmente en las fuentes originales consultadas.")